...
export default function App() {
  ...
  const {balance,setBalance} = React.useContext(AppContext)
  ...
  const {battery,setBattery} = React.useContext(AppContext)
  const [totalBalance, setTotalBalance] = React.useState(0);
  const {totalBattery, setTotalBattery} = React.useContext(AppContext)
  ...
  return (
    <View style={styles.container}>
      <Button onPress={startForegroundUpdate} title="Iniciar"  style={styles.button}/>
      <Button onPress={stopTracking} title="Encerrar" style={styles.button} />
    </View>
  );
}
